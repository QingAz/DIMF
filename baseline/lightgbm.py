...
# Avoid circular import with this file name: baseline/lightgbm.py
...
try:
    ...
    raise ImportError("lightgbm is not installed. Please run: python -m pip install lightgbm") from e
...
def main() -> None:
    ...
    lgb_params_base = {
        "n_estimators": int(lgb_cfg.get("n_estimators", 500)),
        ...
        "subsample": float(lgb_cfg.get("subsample", 1.0)),
        "colsample_bytree": float(lgb_cfg.get("colsample_bytree", 1.0)),
    ...
    for seed in seeds:
        print(f"\n########## Seed {seed} ##########")
        lgb_params = dict(lgb_params_base)
        lgb_params["random_state"] = int(seed)
    ...
    out_dir = PROJECT_ROOT / "outputs" / "baseline_lightgbm"
