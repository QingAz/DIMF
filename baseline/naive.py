...
def build_targets_and_naive_preds(y: np.ndarray, L: int, H: int) -> Tuple[np.ndarray, np.ndarray]:
    t_min = L - 1
    t_max = len(y) - H - 1
    if t_max < t_min:
    ...
    for t in range(t_min, t_max + 1):
        y_true_rows.append(y[t + 1 : t + H + 1])
        # Naive persistence: predict all future steps with latest observed value y[t]
        y_pred_rows.append(np.full((H,), y[t], dtype=np.float32))
...
def main() -> None:
    ...
    yva_true, yva_pred = build_targets_and_naive_preds(prepared.y_val, L=L, H=H)
    yte_true, yte_pred = build_targets_and_naive_preds(prepared.y_test, L=L, H=H)

    yva_true_inv = inverse_scale_2d(yva_true, prepared.scaler_y)
    yva_pred_inv = inverse_scale_2d(yva_pred, prepared.scaler_y)
    yte_true_inv = inverse_scale_2d(yte_true, prepared.scaler_y)
    yte_pred_inv = inverse_scale_2d(yte_pred, prepared.scaler_y)

    val_metrics = evaluate(yva_true_inv, yva_pred_inv)
    test_metrics = evaluate(yte_true_inv, yte_pred_inv)

    per_seed = []
    mae_vals, rmse_vals, r2_vals = [], [], []

    for seed in seeds:
        # Naive baseline is deterministic; keep seed loop for unified reporting protocol.
    ...
    out_dir = PROJECT_ROOT / "outputs" / "baseline_naive"
    ...
    cols = {}
    for i in range(H):
        cols[f"y_true_t{i + 1}"] = yte_true_inv[:, i]
    for i in range(H):
        cols[f"y_pred_t{i + 1}"] = yte_pred_inv[:, i]
