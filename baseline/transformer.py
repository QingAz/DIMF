...
class TransformerBaseline(nn.Module):
    def __init__(
        ...
        self.pos = PositionalEncoding(d_model=d_model, max_len=max_len + 8)
...
def main() -> None:
    ...
    d_model = int(model_cfg.get("hidden_dim", 64))
    nhead = int(model_cfg.get("transformer_nhead", 4))
    num_layers = int(model_cfg.get("num_layers", 3))
    dim_feedforward = int(model_cfg.get("transformer_ff_dim", 256))
    ...
    out_dir = PROJECT_ROOT / "outputs" / "baseline_transformer"
    ...
    print("=== Transformer Baseline Done ===")
    print(f"L={L}, H={H}")
    print("Seeds:", seeds)
    print(
