...
# Avoid circular import with this file name: baseline/xgboost.py
...
try:
    ...
    raise ImportError("xgboost is not installed. Please run: python -m pip install xgboost") from e
...
def train_and_predict(
    ...
    H = ytr.shape[1]
    yva_hat = np.zeros_like(yva, dtype=np.float32)
    yte_hat = np.zeros((Xte.shape[0], H), dtype=np.float32)

    for h in range(H):
        print(f"\n=== Training horizon t+{h + 1}/{H} ===")
        model = XGBRegressor(**xgb_params)
        model.fit(
            Xtr,
            ytr[:, h],
            eval_set=[(Xtr, ytr[:, h]), (Xva, yva[:, h])],
            verbose=verbose_eval,
        )
        yva_hat[:, h] = model.predict(Xva).astype(np.float32)
        yte_hat[:, h] = model.predict(Xte).astype(np.float32)
    return yva_hat, yte_hat
...
def main() -> None:
    ...
    for seed in seeds:
        ...
        xgb_params["random_state"] = int(seed)

        yva_hat, yte_hat = train_and_predict(
            Xtr=Xtr,
            ytr=ytr,
            Xva=Xva,
            yva=yva,
    ...
    out_dir = PROJECT_ROOT / "outputs" / "baseline_xgboost"
