...
class PreparedData:
    ...
    segment_ids_train: Optional[np.ndarray] = None
    segment_ids_val: Optional[np.ndarray] = None
    segment_ids_test: Optional[np.ndarray] = None

def _is_aux_target_col(col: str) -> bool:
    if col in {
        "lag_gt",
        "lag_expected_gt",
        "lag_flag",
    ...
    return col.endswith(suffixes) or "_true_pi_lag" in col


def _group_for_col(col: str, feed_prefix: str, s1_prefix: str, s2_prefix: str, s3_prefix: str) -> Optional[str]:
    if col.startswith(feed_prefix):
        return "feed"
    if col.startswith(s1_prefix):
        return "stage1"
    if col.startswith(s2_prefix):
        return "stage2"
    if col.startswith(s3_prefix):
        return "stage3"
    return None


def _infer_groups(df: pd.DataFrame, time_col: str, target_cols: List[str],
                  feed_prefix: str, s1_prefix: str, s2_prefix: str, s3_prefix: str):
    # 通过列名前缀把原始变量归入四个工段，time/target 列不参与分组。
    excluded = {time_col, *target_cols}
    cols = [c for c in df.columns if c not in excluded and not _is_aux_target_col(c)]
    groups = {"feed": [], "stage1": [], "stage2": [], "stage3": []}
    for c in cols:
        group_name = _group_for_col(c, feed_prefix, s1_prefix, s2_prefix, s3_prefix)
...
def _resolve_target_cols(
    ...
    if target_cols is None:
        ...
        if raw in prefix_by_group:
            cols = [
                col
                for col in df.columns
                if col.startswith(prefix_by_group[raw]) and not _is_aux_target_col(col)
            ]
        else:
...
def load_and_prepare(
    ...
    target_cols_resolved = _resolve_target_cols(
        df,
        target_col=target_col,
        target_cols=target_cols,
        feed_prefix=feed_prefix,
        s1_prefix=stage1_prefix,
        s2_prefix=stage2_prefix,
        s3_prefix=stage3_prefix,
    )

    groups = _infer_groups(df, time_col, target_cols_resolved, feed_prefix, stage1_prefix, stage2_prefix, stage3_prefix)
    if include_target_history:
        for col in target_cols_resolved:
            group_name = _group_for_col(col, feed_prefix, stage1_prefix, stage2_prefix, stage3_prefix)
            if group_name is not None and col not in groups[group_name]:
                groups[group_name] = groups[group_name] + [col]
    ...
    scaler_y = StandardScaler().fit(df_train[target_cols_resolved].values)
    ...
    def transform(df_part):
        ...
        y = scaler_y.transform(df_part[target_cols_resolved].values).astype(np.float32)
        if len(target_cols_resolved) == 1:
            y = y.reshape(-1)
        return X_groups, y
    ...
    def extract_extra_targets(df_part: pd.DataFrame) -> Optional[Dict[str, np.ndarray]]:
        ...
        extra_targets = {}
        sample_index = (
            df_part["sample_index"].fillna(-1).astype(np.int64).to_numpy()
            if "sample_index" in df_part.columns
            else np.arange(len(df_part), dtype=np.int64)
        )
        time_index = (
            df_part["time_index"].fillna(-1).astype(np.int64).to_numpy()
            if "time_index" in df_part.columns
            else sample_index
        )
        extra_targets["sample_index"] = sample_index
        extra_targets["time_index"] = time_index
        extra_targets["stage1_to_stage2_sample_index"] = sample_index
        extra_targets["stage1_to_stage2_time_index"] = time_index
        if "segment_id" in df_part.columns:
            segment_id = df_part["segment_id"].fillna(-1).astype(np.int64).to_numpy()
            extra_targets["segment_id"] = segment_id
            extra_targets["stage1_to_stage2_segment_id"] = segment_id
        region_col = "region_id" if "region_id" in df_part.columns else "tile_id" if "tile_id" in df_part.columns else None
        if region_col is not None:
            region_id = df_part[region_col].fillna(-1).astype(np.int64).to_numpy()
            extra_targets["region_id"] = region_id
            extra_targets["stage1_to_stage2_region_id"] = region_id
        if "lag_gt" in df_part.columns:
            extra_targets["stage1_to_stage2_lag_gt"] = (
                df_part["lag_gt"].fillna(-1).astype(np.int64).to_numpy()
            )
        if "lag_expected_gt" in df_part.columns:
            extra_targets["stage1_to_stage2_lag_expected_gt"] = (
                df_part["lag_expected_gt"].fillna(-1).astype(np.float32).to_numpy()
            )
        if "lag_flag" in df_part.columns:
            extra_targets["stage1_to_stage2_lag_flag"] = (
                df_part["lag_flag"].fillna(0).astype(np.int64).to_numpy()
            )
        if "shape_id" in df_part.columns:
            extra_targets["stage1_to_stage2_shape_id"] = (
                df_part["shape_id"].fillna(0).astype(np.int64).to_numpy()
            )
        for edge in ("feed_to_stage1", "stage1_to_stage2", "stage2_to_stage3"):
            lag_col = f"{edge}_lag_gt"
            if lag_col in df_part.columns:
                extra_targets[f"{edge}_lag_gt"] = (
                    df_part[lag_col].fillna(-1).astype(np.int64).to_numpy()
                )
            expected_col = f"{edge}_lag_expected_gt"
            if expected_col in df_part.columns:
                extra_targets[f"{edge}_lag_expected_gt"] = (
                    df_part[expected_col].fillna(-1).astype(np.float32).to_numpy()
                )
            flag_col = f"{edge}_lag_flag"
            if flag_col in df_part.columns:
                extra_targets[f"{edge}_lag_flag"] = (
                    df_part[flag_col].fillna(0).astype(np.int64).to_numpy()
                )
            shape_col = f"{edge}_shape_id"
            if shape_col in df_part.columns:
                extra_targets[f"{edge}_shape_id"] = (
                    df_part[shape_col].fillna(0).astype(np.int64).to_numpy()
                )
            pi_prefix = f"{edge}_true_pi_lag"
            pi_cols = [col for col in df_part.columns if col.startswith(pi_prefix)]
            if pi_cols:
                pi_cols = sorted(pi_cols, key=lambda name: int(name.split("lag")[-1]))
                extra_targets[f"{edge}_lag_soft_gt"] = (
                    df_part[pi_cols].fillna(0.0).astype(np.float32).to_numpy()
                )
    ...
    prepared = PreparedData(
        ...
        segment_ids_train=df_train["segment_id"].to_numpy() if "segment_id" in df_train.columns else None,
        segment_ids_val=df_val["segment_id"].to_numpy() if "segment_id" in df_val.columns else None,
        segment_ids_test=df_test["segment_id"].to_numpy() if "segment_id" in df_test.columns else None,
    )
