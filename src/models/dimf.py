...
class DIMF(nn.Module):
    ...
    def __init__(
        ...
        head_stage: str = "stage3",
        use_feed_context: bool = True,
        output_dim: int = 1,
        delay_prior_lambda: float = 1.0,
        delay_prior_mode: str = "none",
        delay_prior_sigma: float = 1.5,
    ):
        super().__init__()
        ...
        self.head_stage = str(head_stage).lower()
        if self.head_stage not in {"stage2", "stage3"}:
            raise ValueError("head_stage must be 'stage2' or 'stage3'")
        self.use_feed_context = bool(use_feed_context)
        self.output_dim = int(output_dim)
        self.delay_prior_lambda = float(delay_prior_lambda)
        self.delay_prior_mode = str(delay_prior_mode or "none")
        self.delay_prior_sigma = float(delay_prior_sigma)
        # forward 后会把每条工段边的对齐中间量缓存在这里，
        # 训练循环可以直接读取，避免重复前向。
        self.latest_alignment_cache = {}
        self.latest_lag_output = None
        # The STDA module is registered here as DIMF's lag-guided delay-prior
        # generator. Keep the historical attribute name so existing checkpoints
        # with lag_identifier.* keys remain loadable.
        self.lag_identifier = None
        self.lag_guided_edge_name = None
        self.lag_guided_source_stage = None
        self.lag_guided_target_stage = None
        self.lag_guided_feature_mask = None
        self.lag_guided_prior_cfg = {
            "lambda_prior": self.delay_prior_lambda,
            "prior_mode": self.delay_prior_mode,
            "sigma_prior": self.delay_prior_sigma,
            "weak_prior_mix": 0.0,
        }
    ...
    def attach_lag_guided_prior_generator(
        self,
        prior_generator: nn.Module,
        edge_name: str = "stage1_to_stage2",
        source_stage: str = "stage1",
        target_stage: str = "stage2",
        ...
        """Register the STDA prior generator inside DIMF's alignment block.

        The attached module estimates a feature-level lag distribution from the
        raw source/target stage windows. DIMF then injects that distribution as
        a soft prior into DelayAlignment, which is the part that produces the
        aligned upstream message used by the y-prediction path.
        """
        self.lag_identifier = prior_generator
        self.lag_guided_edge_name = str(edge_name)
        self.lag_guided_source_stage = str(source_stage)
        self.lag_guided_target_stage = str(target_stage)
        self.lag_guided_feature_mask = feature_mask
        self.lag_guided_prior_cfg = {
            "lambda_prior": self.delay_prior_lambda if lambda_prior is None else float(lambda_prior),
            "prior_mode": self.delay_prior_mode if prior_mode is None else str(prior_mode),
            "sigma_prior": self.delay_prior_sigma if sigma_prior is None else float(sigma_prior),
            "weak_prior_mix": float(weak_prior_mix),
        }

    def has_lag_guided_alignment(self) -> bool:
        return self.lag_identifier is not None

    def build_lag_guided_delay_priors(
        self,
        lag_output: Dict[str, torch.Tensor],
    ) -> Dict[str, Dict[str, object]]:
        if self.lag_guided_edge_name is None:
            raise RuntimeError("Lag-guided prior generator is not attached")
        pi_prior = lag_output["pi_lag"]
        feature_mask = self.lag_guided_feature_mask
        if feature_mask is not None:
            if pi_prior.ndim != 3:
                raise ValueError("Lag-guided pi_prior must be shaped [B, d_source, K]")
            mask = feature_mask.to(device=pi_prior.device, dtype=torch.bool)
            if mask.numel() != pi_prior.shape[1]:
                raise ValueError("Lag-guided feature_mask length does not match source feature dimension")
            weak_mix = float(self.lag_guided_prior_cfg.get("weak_prior_mix", 0.0))
            screened = pi_prior.clone()
            uniform = torch.full_like(screened[:, ~mask, :], 1.0 / float(pi_prior.shape[-1]))
            if weak_mix <= 0.0:
                screened[:, ~mask, :] = uniform
            else:
                screened[:, ~mask, :] = weak_mix * screened[:, ~mask, :] + (1.0 - weak_mix) * uniform
            pi_prior = screened
        return {
            self.lag_guided_edge_name: {
                "pi_prior": pi_prior,
                "lambda_prior": float(self.lag_guided_prior_cfg["lambda_prior"]),
                "prior_mode": str(self.lag_guided_prior_cfg["prior_mode"]),
                "sigma_prior": float(self.lag_guided_prior_cfg["sigma_prior"]),
            }
        }

    def predict_lag_prior(
        self,
        X_seq: Dict[str, torch.Tensor],
        segment_id: Optional[torch.Tensor] = None,
    ) -> Dict[str, torch.Tensor]:
        if self.lag_identifier is None:
            raise RuntimeError("No lag-guided prior generator is attached to DIMF")
        if self.lag_guided_source_stage is None or self.lag_guided_target_stage is None:
            raise RuntimeError("Lag-guided prior edge stages are not configured")
        return self.lag_identifier(
            X_seq[self.lag_guided_source_stage],
            target_seq=X_seq[self.lag_guided_target_stage],
            segment_id=segment_id,
        )

    def infer_lag_guided_delay_priors(
        self,
        X_seq: Dict[str, torch.Tensor],
        segment_id: Optional[torch.Tensor] = None,
    ) -> Tuple[Dict[str, torch.Tensor], Dict[str, Dict[str, object]]]:
        lag_output = self.predict_lag_prior(X_seq, segment_id=segment_id)
        self.latest_lag_output = lag_output
        return lag_output, self.build_lag_guided_delay_priors(lag_output)

    def forward_with_lag_guided_alignment(
        self,
        X_seq: Dict[str, torch.Tensor],
        segment_id: Optional[torch.Tensor] = None,
    ) -> Tuple[torch.Tensor, Dict[str, torch.Tensor], Dict[str, torch.Tensor]]:
        lag_output, delay_priors = self.infer_lag_guided_delay_priors(X_seq, segment_id=segment_id)
        y_hat, alignment_pi = self.forward(X_seq, delay_priors=delay_priors)
        return y_hat, alignment_pi, lag_output
    ...
    def _zero_lag_pi(self, reference_seq: torch.Tensor) -> torch.Tensor:
        pi = reference_seq.new_zeros(reference_seq.shape[0], reference_seq.shape[1], self.L_max + 1)
        pi[..., 0] = 1.0
        return pi

    def _edge_prior_kwargs(
        self,
        edge_name: str,
        delay_priors: Optional[Dict[str, object]],
        lambda_prior: Optional[float],
        prior_mode: Optional[str],
        sigma_prior: Optional[float],
    ) -> Dict[str, object]:
        kwargs = {
            "lambda_prior": self.delay_prior_lambda if lambda_prior is None else float(lambda_prior),
            "prior_mode": self.delay_prior_mode if prior_mode is None else str(prior_mode),
        ...
        if not delay_priors or edge_name not in delay_priors:
            return kwargs
        payload = delay_priors[edge_name]
        if torch.is_tensor(payload):
            kwargs["pi_prior"] = payload
            return kwargs
        if not isinstance(payload, dict):
            raise TypeError(f"Delay prior for {edge_name} must be a tensor or dict")
        if "pi_prior" in payload:
            kwargs["pi_prior"] = payload["pi_prior"]
        if "d_prior" in payload:
            kwargs["d_prior"] = payload["d_prior"]
        if "lambda_prior" in payload:
            kwargs["lambda_prior"] = float(payload["lambda_prior"])
        if "prior_mode" in payload:
            kwargs["prior_mode"] = str(payload["prior_mode"])
        if "sigma_prior" in payload:
            kwargs["sigma_prior"] = float(payload["sigma_prior"])
        return kwargs

    def forward(
        self,
        X_seq: Dict[str, torch.Tensor],
        delay_priors: Optional[Dict[str, object]] = None,
        lambda_prior: Optional[float] = None,
        prior_mode: Optional[str] = None,
        sigma_prior: Optional[float] = None,
        ...
        """
        if self.head_stage == "stage2" and not self.use_feed_context:
            E1_seq, E1_last = self.enc_s1(X_seq["stage1"])
            E2_seq, E2_last = self.enc_s2(X_seq["stage2"])
            del E1_last, E2_last

            pi01 = self._zero_lag_pi(E1_seq)
            raw1_seq = E1_seq
            m2_seq, pi12, raw2_seq = self.align_1_2.forward_seq(
                E2_seq,
                E1_seq,
                **self._edge_prior_kwargs(
                    "stage1_to_stage2",
                    delay_priors,
                    lambda_prior,
                    prior_mode,
            ...
            self.latest_alignment_cache = {
                ...
                "stage2_to_stage3": {
                    "active": False,
                    "pi": pi23,
                    "aligned_upstream_raw": u2_seq,
                    "downstream_seq": u2_seq,
                },
            }
            y_hat = self.head(u2_seq[:, -1, :])
            if self.output_dim == 1:
                y_hat = y_hat.squeeze(-1)
            return y_hat, {
                "feed_to_stage1": pi01,
                "stage1_to_stage2": pi12,
                "stage2_to_stage3": pi23,
            }
        ...
        m1_seq, pi01, raw1_seq = self.align_0_1.forward_seq(
            E1_seq,
            E0_seq,
            **self._edge_prior_kwargs("feed_to_stage1", delay_priors, lambda_prior, prior_mode, sigma_prior),
        )   # feed -> stage1
        m1_seq = self._apply_alignment_dropout(m1_seq)
        u1_seq = self.fuse_s1(E1_seq, m1_seq)
        if not self.use_feed_context:
            pi01 = self._zero_lag_pi(E1_seq)
            raw1_seq = E1_seq
            u1_seq = E1_seq
        ...
        m2_seq, pi12, raw2_seq = self.align_1_2.forward_seq(
            E2_seq,
            u1_seq,
            **self._edge_prior_kwargs("stage1_to_stage2", delay_priors, lambda_prior, prior_mode, sigma_prior),
        )   # stage1 -> stage2
        m2_seq = self._apply_alignment_dropout(m2_seq)
        u2_seq = self.fuse_s2(E2_seq, m2_seq)
        ...
        m3_seq, pi23, raw3_seq = self.align_2_3.forward_seq(
            E3_seq,
            u2_seq,
            **self._edge_prior_kwargs("stage2_to_stage3", delay_priors, lambda_prior, prior_mode, sigma_prior),
        )   # stage2 -> stage3
        m3_seq = self._apply_alignment_dropout(m3_seq)
        u3_seq = self.fuse_s3(E3_seq, m3_seq)
        u3 = u3_seq[:, -1, :]
        if self.head_stage == "stage2":
            pi23 = self._zero_lag_pi(u2_seq)
            raw3_seq = u2_seq
            E3_seq = u2_seq
            u3 = u2_seq[:, -1, :]
        ...
        self.latest_alignment_cache = {
            "feed_to_stage1": {
                "active": self.align_feed_to_stage1 and self.use_feed_context,
                "pi": pi01,
                "aligned_upstream_raw": raw1_seq,
                "downstream_seq": E1_seq,
            },
            ...
            "stage2_to_stage3": {
                "active": self.align_stage2_to_stage3 and self.head_stage == "stage3",
                "pi": pi23,
